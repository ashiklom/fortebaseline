...
Instead, we selected a subset of parameters guided by (but expanding upon) previous ED-2.2 sensitivity studies [@dietze_2014_quantitative; @raczka_2018_what; @viskari_2019_influence].
...
kable(tbl, escape = is_latex_output(), caption = paste0(
...
Driving meteorological data was 6-hourly CRU-NCEP <!-- TODO ref. More here? --> combined with an annual atmospheric CO~2~ record from Law-Dome ice core [@etheridge_1998_historical] and Mauna Loa observatory [@thoning_1989_atmospheric] linearly interpolated to 3-hourly resolution.
...
To understand the physical and ecological mechanisms underlying these differences, we also compared the cohort-level composition, height, and canopy radiation profiles in these simulations in four stages of the simulation (Figure 2).

Second, we compared simulations with ED-2.2 default parameters against simulations using median parameter estimates from a trait meta-analysis (Figure 3).
Here, we looked predictions of annual mean C fluxes (gross and net primary productivity, and autotrophic and heterotrophic respiration; Figure 4) and of summer LAI by PFT (Figure 5).
...
We look at these variables independently as time series (Figure 6) and as joint predictions in three years (Figure 7).
We then consider impacts on composition by looking at predicted PFT fractions of total aboveground biomass (AGB) at four different years in the simulation (Figure 8) and at time series of LAI by PFT for five parameter combinations spanning the range of predicted ecological outcomes (Figure 9).

Finally, we assessed which parameters contributed the most to parameter uncertainty for each model structure.
We did this by performing a sensitivity and variance decomposition analysis following @lebauer_2013_facilitating, @dietze_2014_quantitative, and @raczka_2018_what.
...
First, we assessed how ED-2.2 predictions of plot-level ecosystem properties and composition varied across model structures and parameter values.
For plot-level ecosystem properties, we examined mean annual gross (GPP) and net primary productivity (NPP), and autotrophic (RA) and heterotrophic respiration (RH), as well as total leaf area index (LAI) averaged June-August.
We compared predicted NPP and LAI values against measurements of NPP and LAI at UMBS averaged over years XXXX-YYYY. <!-- TODO -->
Although these observations occurred later than the years of the simulation, we found that the distributions of ED-2.2 predictions of NPP and LAI were relatively stable after 1975 and therefore constituted a reasonable approximation of what ED-2.2 would have predicted out to the present day.
For ease of interpretation, we only show the top eight parameters in terms of partial variance for every model configuration.

For composition, we looked at annual means of summer aboveground biomass and leaf area index
...
Under a closed canopy, switching from a two-stream to a multiple-scatter canopy radiative transfer model also enhanced the understory light environment, but to a lesser extent, and the brightest understory occurred under the combination of the two-stream radiative transfer and finite canopy radius.
...
First, throughout all simulations and across all model structures, autotrophic respiration was several times higher with median parameter estimates compared to default values.
...
Under the default parameterization, ED2 produces forests dominated by Early Hardwoods, with an understory of small mid- and even smaller late-hardwood trees that eventually die out.
...
Exploratory analyses (results not shown) show that the main parameters responsible for this compositional trajectory are SLA (especially the higher SLA of pine and mid-hardwood), Vcmax, and growth respiration.
...
Net primary productivity (NPP) and leaf area index (LAI) varied with both parameter value and model structure (Figure X).
...
within_vs_across_cap <- paste(
  htmlfig(),
  "Comparison of parameter uncertainty within ensembles (colored bars, colored by model type)",
