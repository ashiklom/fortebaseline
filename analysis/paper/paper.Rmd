...
summary_ts_cap <- paste(
  ...
  "and effective number of PFTs (N~PFT,eff~)",
...
Our overarching objective was to investigate how model structure and parameters interact in multidecadal simulations of forest ecosystem productivity and community succession.
...
Modelled community succession, which was variable across model structural and parameter configurations, poorly predicted the observed transition of dominance from early to mixed successional hardwoods during the first century of regrowth.
<!-- TODO Highlight the large variability! -->
...
Filling this canopy void over time are mid-successional _Quercus_ and _Acer_, and later successional _Pinus_ [@scheuermann_2018_???].
However, with the exception of a select few parameter combinations (e.g. C and D in Figure \@ref(fig:lai-pft-plot)), modelled community succession exhibited only modest changes in functional diversity over time.
<!-- TODO Fix from here on -->
Moreover, a single PFT emerged as dominant in a majority of runs, though _which_ PFT dominated depended on parameter combination.
This mismatch between observations and simulations reinforces the difficulty of accurately representing plant functional diversity in vegetation models [@fisher_2015_taking].
Specific to our analysis, our results indicate that ED-2.2 perceives limited niche overlap among PFTs, resulting in an unrealistically low level of PFT coexistence. <!-- TODO REFS -->

Unlike community succession, most model configurations produced similarly realistic C flux successional trajectories.
In particular, the use of median parameter values resulted in century-long C flux dynamics that mirror the trajectory of site-level observations [@gough_2007_legacy] and, more broadly, those from other temperate forests [@law_2001_carbon; @law_2003_changes; @irvine_2004_age; @tang_2009_soil; @kashian_2013_postfire].
In our study, modelled and observed C fluxes generally increased rapidly in the first quarter century before gradually declining, a pattern that is consistent with ecological theory [@odum_1969_strategy_ecosystem_development] and global observations [@pregitzer_2004_carbon; @luyssaert_2008_old; @gough_2016_disturbance].
At odd with site-level observations, however, is the extent of simulated declines in production over time.
...
Most ED-2.2 runs under-predicted both NPP and LAI, both in the peak period (1920-1950) and towards the end of the simulation (1975-1999) (Figures \@ref(fig:summary-ts-plot)).
However, of our five selected representative runs, the two that most closely matched present day composition at UMBS (early hardwood-dominated; **C** and **D** in Figure \@ref(fig:lai-pft-plot)) also closely matched the observed NPP and LAI (Figure \@ref(fig:summary-ts-plot)).
