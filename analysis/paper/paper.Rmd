...
One of the primary drivers of successional change in temperate forests is competition for light <!-- TODO REF -- Bazaaz? -->, and differences in plants' strategies for light acquisition and utilization are a key driver of niche differentiation in these ecosystems <!-- TODO REF -->.
...
Different model structures very closely for the first half of the simulation, at which point they started to diverge.
...
```{r partial-variance-lai, fig.cap = partial_variance_lai_cap}
partial_variance_lai_cap <- paste(
  htmlfig(),
  "Same as above, but for total leaf area index (LAI)."
)
fig("partial-variance-lai.png")
```
...
The parameters that contributed most to overall model predictive uncertainty ("partial variance") varied with model structure, time period considered, and output variable (NPP or LAI).
...
**Ensemble mean trajectory matches traditional understanding of succession**---rapid growth to peak productivity, followed by gradual decline [@odum_1969_strategy_ecosystem_development]---but **happens too quickly** (Figure 2);
...
Different parameter combinations led to **huge ranges in ensemble predictions of ensemble variables** -- NPP ranged from 0 to almost 2x the observed, and LAI was up to 3x observed (though more tightly clustered around zero) (Figure 2).
Moreover, different parameter combinations produced **qualitatively different ecological narratives** (Figure 4).
...
The dominance of parameter uncertainty over structural uncertainty in our results suggests that **targeted data collection should be a higher priority than model development**.

**Parameter uncertainty is not synonymous with sensitivity**.
Many of the parameters to which model outputs are most sensitive are already well constrained by data and do not contribute as much to predictive uncertainty (Figure 6).
...
The relative position of different parameter combinations in multivariate space (Figure 3), the total amount of parameter uncertainty (Figure 5), and the relative importance of different parameters to predictive uncertainty (Figure 6) all differed across model structures (Figure 3).
This is despite the fact that our model structure changes were relatively modest compared to differences between different models.
