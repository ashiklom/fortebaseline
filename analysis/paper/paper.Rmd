...
Different parameter combinations led to **huge ranges in ensemble predictions of ensemble variables**, with NPP and LAI ranging from zero to more than twice the ensemble mean estimates (Figures 6, 7).
Moreover, different parameter combinations produced **qualitatively different ecological narratives** (Figures 8. 9).
...
For example, leaf optical properties (i.e. reflectance, transmittance) are both measurable and highly influential [see also @viskari_2019_influence], and therefore should be a priority for new data collection.
In contrast, "growth respiration factor" is an empirical coefficient and not measurable, but highly influential [see also @dietze_2014_quantitative; @raczka_2018_what], and therefore should be constrained through calibration.
...
For example, ED-2.2 is highly sensitive to SLA [@dietze_2014_quantitative; @pandit_2018_optimizing], but thanks to the large number of SLA observations, we were able to estimate it very precisely (Figure 3) and, as a result, it was not a significant source of uncertainty in our analysis (Figure 10).
At the same time, some parameters have relatively modest impacts on model outputs---for instance, nonlocal dispersal and background mortality rate in our study (Figure 3, 10)---and therefore, reducing their uncertainties is not an effective way to improve model predictions.
...
The consequences of individual model structure changes were *usually* modest, but with several dramatic exceptions (Figure 4).
Moreover, we found bi-directional interactions between structural and parameter uncertainty:
Different parameter values (especially ones far removed from ED-2.2 defaults) influenced ED-2.2 sensitivity to structural changes (Figures 4, 5),
and changes to model structure in turn changed the relative importance of specific parameters to overall uncertainty (Figure 10).
