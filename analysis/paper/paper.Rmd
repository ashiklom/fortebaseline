...
By default, ED-2.2 supports 17 different plant functional types, which divide plant species according to growth form (grass vs. tree), photosynthetic pathway (C3 vs. C4), leaf habit (deciduous vs. evergreen), biome (e.g. temperate vs. tropical), and successional status (e.g. early, mid, late).
...
The species comprising these plant functional types are shown in Table \@ref(tab:pft-species).
...
First, we evaluated the impact of model structure independent of any changes to parameters by comparing predictions of average annual net primary productivity (NPP) and summer (June, July, August) total leaf area index (LAI) across model structures while holding ED-2.2 parameters at their default values.
Second, we then compared these default parameter simulations to simulations using median parameter estimates from a trait meta-analysis.
