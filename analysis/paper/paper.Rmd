...
These differences in parameter values resulted in ED-2.2 runs that differed from the default runs in several important ways (Figures 4, 5).
First, in every model configuration (except finite canopy, two-stream RTM, and static traits), gross primary productivity (GPP) reached a higher peak, and did so earlier under median parameters (Figure 4).
This was driven primarily by higher V~cmax~ and SLA values for most PFTs (Figure 3).
However, this increase in GPP was offset by a large increase in autotrophic respiration (RA; Figure 4), driven primarily by higher growth and leaf respiration rates across all PFTs (notably, the ED-2.2 default is _zero growth respiration_ for all temperate hardwood PFTs; Figure 3).
The resulting net primary productivity (NPP) had similar peak values under default and median parameters, though the temporal patterns were quite different in most model configurations (Figure 4).

Second, the variation across model structures was much higher, with every change in model structure exerting a noticeable effect on flux variability (Figure 4).
...
Third, even where NPP was similar between default and median runs, the underlying forest composition was very different.
