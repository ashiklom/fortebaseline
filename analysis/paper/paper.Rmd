...
readd(structure_compare_default_knit)
```
...
readd(structure_default_light_knit)
```
...
```{r , fig.cap = default_median_cap}
default_median_cap <- paste(
...
readd(default_median_fluxes_knit)
```
...
readd(summary_ts_plot_knit)
```

`r mdcap(summary_ts_cap)`

<!-- - Structure x parameter uncertainty -->
<!--   - Figure: Time series of NPP, LAI, and relative PFT dominance -->
...
The ensemble spread within any single configuration---i.e. the parameter uncertainty---was similar across all configurations; both NPP and LAI varied 5-10 fold at any given timestep depending on parameter values.
